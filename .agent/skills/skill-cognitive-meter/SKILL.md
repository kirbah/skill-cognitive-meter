...
The `risk` string inside the JSON `metrics` must NOT be a brief summary. It must be a comprehensive, 2-3 sentence explanation detailing exactly *why* this creates cognitive load and *how* the model might fail as a result.
      ...
      "metrics": [
        {
          ...
          "score": "HIGH",
          "risk": "Gathering GitHub handles early requires the model to hold this exact string in its working memory across multiple intermediate CLI commands. If the CLI outputs are long, this variable is highly susceptible to being pushed out of the attention window, resulting in a hallucinated or forgotten handle later in the execution stream.",
      ...
      "metrics": [
        {
          ...
          "score": "CRITICAL",
          "risk": "There is a direct structural contradiction between the global WebSearch tool mandate (which requires a trailing sources block) and this local law. The model experiences severe attention-head strain trying to suppress a heavily weighted system instruction, which frequently leads to logic paralysis or formatting corruptions.",
...
Once you have successfully written both the `.md` and `.json` files to the disk, you must execute the following steps in order:

1. **Build the Report**: Use your terminal tool to run the python builder script from the project root:
   `python ${CLAUDE_SKILL_DIR}/scripts/build_report.py`
2. **Notify the User**: Once the script executes successfully, output a final message to the user telling them the analysis is complete. Explicitly instruct them to open the `pages/index.html` file in their web browser to view the interactive cognitive heatmap.
